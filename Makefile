...
install: ## Install all dependencies required for the service.
	...
	@if ! python3 -m pip --version &> /dev/null; then \
		echo "\033[31mError: pip is not installed for Python 3.\033[0m"; \
		echo "Please install pip. You can often do this with:"; \
		echo "  On Debian/Ubuntu: sudo apt-get install python3-pip"; \
		echo "  On Fedora: sudo dnf install python3-pip"; \
		echo "  Or using ensurepip: python3 -m ensurepip --upgrade"; \
		exit 1; \
	fi
	npm i -g vercel
